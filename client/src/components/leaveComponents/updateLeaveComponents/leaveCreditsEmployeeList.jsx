// LeaveCreditsEmployeeList.jsx
import React, { useEffect, useRef, useState, useMemo } from "react";
import { useQuery, keepPreviousData } from "@tanstack/react-query";
import { getEmployees } from "../../../api/employee"; // Adjust import path
...
const LeaveCreditsEmployeeList = ({ setIsEmployeeLoading }) => {
      ...
      >
        ...
        >
          ...
          <p className="text-xs" style={{ color: "var(--app-muted)" }}>
            Select an employee to view or edit balances
          </p>
        </div>
...
const EmployeeList = ({ setIsEmployeeLoading, borderColor }) => {
  ...
  useEffect(() => {
    ...
    if (!hasNavigatedRef.current && data?.data?.length > 0 && !selectedId) {
      ...
      if (firstSorted) {
        navigate(`/app/leave-credits/${firstSorted._id}`, { replace: true });
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 min-w-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 min-w-0">
          <div className="relative flex-1 group min-w-0">
            ...
            <input
              ...
              className="pl-9 pr-8 py-2 w-full border rounded-lg text-sm outline-none transition-all placeholder:text-[color:var(--app-muted)]"
              style={{
                backgroundColor: "var(--app-surface)",
                borderColor: borderColor,
                color: "var(--app-text)",
      ...
      <div
        className="flex-1 min-h-0 overflow-y-auto px-2 py-2 app-scrollbar transition-colors duration-300 ease-out"
          ...
          {isLoading ? (
            ...
            employees.map((item) => {
              ...
              return (
                <li
                  key={item._id}
                  onClick={() => navigate(`/app/leave-credits/${item._id}`)}
...
export default LeaveCreditsEmployeeList;
